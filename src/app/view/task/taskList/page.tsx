...
import {AcceptButton} from '@/components/task/actionsTask';
import {
  Table,
  TableBody,
  TableCell,
...
import DeleteTaskComponent from '@/components/task/DeleteTaskComponent';
type Task = {
  id: string;
  title: string;
  description: string;
...
const Page = async () => {
  ...
  const response = await GetTaskList(token as string);
  const tasks: Task[] = response?.TaskList || [];
  // const AcceptTask=async(taskId:string)=>{
  //   const response= await AcceptTaskApi(token as string, taskId);
  //   toast.success("Task Accepted Successfully");
  //   // await AcceptTask(token as string, taskId);
  // }
          ...
          {tasks.length > 0 ? (
            tasks.map((task) => (
              <TableRow key={task.id}>
                ...
                <TableCell className="font-medium border-2 border-gray-100 text-start">₹ {task.TotalProjectCost?? 0} </TableCell>
                ...
                <TableCell className="border-2 border-gray-100 text-start">
                  ...
                  <div className='space-x-4'>
                    <Link href={`/view/task/completedTaskDetail/${task.id}`}><Button>View</Button></Link>
                    <AcceptButton taskId={task.id} token={token as string}/>
                    
                  </div>
                  
                
                   :null}
                   {task.status ==="PENDING"? <DeleteTaskComponent id={task.id}/>:null}
                   </TableCell>
