import { GetAssignTaskList } from '@/api/task';
...
const Page = async () => {
  ...
  const response = await GetAssignTaskList(token as string);
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-semibold mb-4">My Task List</h1>
            ...
            tasks.map((task) => (
              <TableRow key={task.id}>
                ...
                <TableCell className="border border-gray-300"> 
                  ...
                  task.status =="PENDING" &&
                   (<>
                   <Link href={`/view/task/completeTask/${task.id}`}><Button variant="link">Complete Task</Button></Link> 
